...
objHttpServletRequest.login(request.getParameter("username"), request.getParameter("password"));
	...
	throws IOException, ServletException {
	if (debug) log("SecurityTokenFilter:DoAfterProcessing");
System.out.println("After Filter");
HttpServletRequest objHttpServletRequest = (HttpServletRequest) request;
 HttpServletResponse objHttpServletResponse = (HttpServletResponse) response;
// objHttpServletRequest.login(request.getParameter("username"), request.getParameter("password"));
System.out.println(objHttpServletRequest.isUserInRole("admin"));

 if(objHttpServletRequest.isUserInRole("admin"))
    objHttpServletResponse.sendRedirect("admin.jsp");
else if(objHttpServletRequest.isUserInRole("supervisor"))
    objHttpServletResponse.sendRedirect("users.jsp");
 else
