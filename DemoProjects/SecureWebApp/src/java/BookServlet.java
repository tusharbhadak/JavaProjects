...
import ejb.SecureBookBeanRemote;
import service.BookMaster;
...
import service.SecureBookBeanService;

public class BookServlet extends HttpServlet {

    @WebServiceRef(wsdlLocation = "https://localhost:8181/SecureBookBeanService/SecureBookBean?WSDL")
    private SecureBookBeanService service;
    service.SecureBookBean book;
   
  //  @EJB(mappedName="ejb/securebean")
   //SecureBookBeanRemote book;
    ...
    public void service(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException {
        ...
        try { // Call Web Service Operation
           book = service.getSecureBookBeanPort();
((BindingProvider)book).getRequestContext().put(
   BindingProvider.ENDPOINT_ADDRESS_PROPERTY, "https://localhost:8181/SecureBookBeanService/SecureBookBean?WSDL");
  
  ((BindingProvider)book).getRequestContext().put(
   BindingProvider.USERNAME_PROPERTY, "admin");

  ((BindingProvider)book).getRequestContext().put(
    BindingProvider.PASSWORD_PROPERTY,  "admin");
        ...
        if("D".equals(request.getParameter("hidMode"))) {
            try {
              ...
              book.removeBook(new Integer(st.nextToken()));
                ...
                response.sendRedirect("/SecureWebApp/BookServlet");
            } catch(Exception e) {
 ...
 book.updateBook(new Integer(request.getParameter("hidBookID")),bookname,authorname,publishername,synopsis);
        ...
        out.println("<FORM ACTION='/SecureWebApp/BookServlet' METHOD='post' NAME='frmBook'>");
        ...
        if(book != null) {
            try {
            //List<entity.BookMaster> list = book.getAllBooks();
           // List<BookMaster> list = (List<BookMaster>)book.getAllBooks();
            ...
            out.println("<TABLE ALIGN='center' BORDER='1' BORDERCOLOR='skyblue' CELLPADDING='0' CELLSPACING='0' WIDTH='50%' NAME='tblSecondChild'>");
              ...
              //  book.getAllBooks().toString();
               //System.out.println("In Servlet -- "); 
               books= (List<BookMaster>)book.getAllBooks();
               System.out.println("In Servlet -- "+books.toString()); 
